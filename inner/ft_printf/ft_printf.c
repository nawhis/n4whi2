...
/*   Updated: 2023/11/10 19:57:49 by sihkang          ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_putaddr(void *ad, s_option *opt, s_string *str)
{
	...
	size = len;
	// if (opt->flag[1] || opt->flag[2] || opt->flag[3] || opt->flag[4] || opt->flag[5])
	// 	return (0);
	if (opt->width > len + 2)
	{
...
char	*ft_unumtostr(unsigned int num, size_t *len, s_option *opt)
{
	...
	if (opt->flag[1] || opt->flag[3]) // 부호 플래그 또는 공백 플래그 존재 시
		(*len)++;
